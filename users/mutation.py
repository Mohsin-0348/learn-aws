...
from bases.utils import create_token, generate_auth_key
from mysite.authentication import TokenManager
from mysite.permissions import is_authenticated
from users.choices import IdentifierBaseChoice, RoleChoice
from users.forms import ClientForm, UserRegistrationForm
from users.login_backend import signup
from users.models import Client, ResetPassword
from users.object_types import ClientType, UserType
...
class ClientMutation(DjangoFormMutation):
    ...
    def mutate_and_get_payload(self, info, **input):
        ...
        if form.is_valid():
            ...
            if form.cleaned_data['identifier_base'] == IdentifierBaseChoice.IDENTIFIER_BASED \
                    and not form.cleaned_data['identifier_model_name']:
                raise GraphQLError(
